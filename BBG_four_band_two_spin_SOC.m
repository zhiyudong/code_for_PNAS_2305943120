...
u_list = linspace(35,52,100); %70; % 10,70,40

% 35,55,100 for lambda=7
% 30,50,100 for lambda=8
n_list = linspace(-6.7E-3,-10.5E-3,100); %-4E-3; % TOTAL carrier density (two spins two valleys)
%
...
%         hole_pocket_num_Kp_dn = Count_hole_pockets_Kp_dn.NumObjects;
        ...
        eh_pockets_num_K_up(i_n,i_u) = hole_pocket_num_K_up + el_pocket_num_K_up -1;
        eh_pockets_num_K_dn(i_n,i_u) = hole_pocket_num_K_dn + el_pocket_num_K_dn -1;
...
%% plot FS
        ...
        subplot(1,2,1)
        contour_range=[mu,mu];
        contour3(kx_mesh,ky_mesh,epsilon0K_up(:,:,conduction_band_K_up)',contour_range,'r')
        hold on
        contour3(kx_mesh,ky_mesh,epsilon0Kp_dn(:,:,conduction_band_Kp_up)',contour_range,'b')
        hold off
        axis equal
       xticks([-0.2,0,0.2])
        yticks([-0.2,0,0.2])
...
% 45 for lambda = 7 nkxy=150
% 42.5 for lambda = 8 nkxy=60

D_measured = 0.95;
u_vs_D = u_measured/D_measured;

[num,txt,raw] = xlsread('extract_WSe2+BBG_phase_boundary.xls');
n_extract1=num(:,1);
D_extract1=num(:,2);
u_extract1 = u_vs_D*D_extract1;

[num,txt,raw] = xlsread('extract_WSe2+BBG_SC_phase_all.xls');
n_extract2=num(:,1);
D_extract2=num(:,2);
...
hold on
%plot3(n_extract1,u_extract1,1000*ones(size(u_extract1)),...
%    'o','Color','k','MarkerSize',4,'MarkerFaceColor','none')
%plot3(n_extract2,u_extract2,1000*ones(size(u_extract2)),...
%    'o','Color','b','MarkerSize',4,'MarkerFaceColor','cyan')
plot3(n_extract1,u_extract1,1000*ones(size(u_extract1)),...
    '-','Color','k','linewidth',3)
plot3(n_extract2,u_extract2,1000*ones(size(u_extract2)),...
...
plot3([-0.00995;-0.00962],u_measured*ones(2,1), 1000*ones(2,1),...
    'o','Color','k','MarkerSize',6,'MarkerFaceColor',[0.7,0,0]) % two transition points on the line scanned
